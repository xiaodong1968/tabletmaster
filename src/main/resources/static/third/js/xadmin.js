...
layui.use(['layer','element','jquery'],function() {
     ...
     * author:吴健
     * date:2022/05/10
     * 1.第一次加载二级菜单时，根据父菜单的ID去后台查询，并通过html拼接，加入到导航菜单中
     * 2.如果某个导航菜单的二级菜单已经被加载，则直接展开不再访问后台
     */
    $('.left-nav #nav').on('click', 'li', function(event) {
        var hasLoad=$(this).find("input[type='hidden']").eq(1).val();//判断子菜单是否已经被加载
        if(hasLoad==0){
            var $$this=$(this);
            var menuId=$(this).find("input[type='hidden']").eq(0).val();//找到当前菜单的ID
            $.ajax({
                url:'queryMenuByParentId',
                data:{"parentId":menuId},
                success:function (data) {
                    var htm="<ul class='sub-menu' style='display: block'>";
                    for(var i=0;i<data.length;i++){
                        var href=data[i].href+"?menuId="+data[i].id;
                        htm=htm+"<li><a onclick=\"xadmin.add_tab('"+data[i].name+"','"+href+"')\">" +
                            " <i class='fa "+data[i].icon+"'></i><cite>"+data[i].name+"</cite></a></li>";
                    }
                    htm=htm+"</ul>";
                    $(htm).appendTo($($$this));
                    ...
                    if($($$this).children('.sub-menu').length){
                        if($($$this).hasClass('open')){
                            ...
                            $($$this).find('.nav_right').removeClass("fa-chevron-down").addClass("fa-chevron-right");
                            ...
                            $($$this).find('.nav_right').removeClass("fa-chevron-right").addClass("fa-chevron-down");
                            $($$this).siblings().find('.nav_right').removeClass("fa-chevron-down").addClass("fa-chevron-right");
                            $($$this).siblings().removeClass('open');
                        }
                    }
                    $($$this).find("input[type='hidden']").eq(1).val(1);
                }
            });
