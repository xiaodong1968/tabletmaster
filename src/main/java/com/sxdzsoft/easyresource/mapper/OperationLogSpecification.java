...
public class OperationLogSpecification implements Specification<OperationLog> {
    ...
    public OperationLogSpecification(OperationLog operationLog) {
        this.operationLog=operationLog;
    }
    @Override
    public Predicate toPredicate(Root root, CriteriaQuery query, CriteriaBuilder criteriaBuilder) {
        List<Predicate> predicates = new ArrayList<Predicate>();
        if(!operationLog.getLevel_string().equals("-1")) {
            predicates.add(criteriaBuilder.equal(root.get("level_string").as(String.class), operationLog.getLevel_string()));
        }
